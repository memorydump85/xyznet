    ...
    const std::size_t offset )
{
    ...
    const auto T = mathx::xyzrph_to_matrix(pos, euler_rotation);
...
int main(int argc, char *argv[]) {
    ...
    for (std::size_t i=0; i < trajectory.vertices.size(); ++i) {
        ...
        const auto &euler_rotation = trajectory.normals[i];

        std::cout << strfmt("\n\n# trajectory position #%lu\n", i);
        vx_offset += dbg__output_marker_(pos, euler_rotation, vx_offset);
        std::cout << '\n';

        const auto &scan_dirs = LidarSensorScanGenerator::get_scan(pos, euler_rotation);
